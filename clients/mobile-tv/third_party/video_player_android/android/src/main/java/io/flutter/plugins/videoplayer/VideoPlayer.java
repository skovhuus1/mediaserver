...
public abstract class VideoPlayer implements VideoPlayerInstanceApi {
  ...
  public void setBoltBytesAutoMaximumHeight(long maximumHeight) {
    if (trackSelector == null) {
      throw new IllegalStateException("Cannot cap video quality: track selector is null");
    }
  ...
  @UnstableApi
  public @NonNull Map<String, Object> getBoltBytesTelemetry() {
    Map<String, Object> values = new HashMap<>();
    Format format = exoPlayer.getVideoFormat();
    long position = Math.max(0, exoPlayer.getCurrentPosition());
    long buffered = Math.max(position, exoPlayer.getBufferedPosition());
    values.put("positionMs", position);
    ...
    if (format != null) {
      ...
      if (format.bitrate != Format.NO_VALUE) values.put("bitrate", format.bitrate);
      if (format.codecs != null) values.put("decoder", format.codecs);
    }
    DecoderCounters counters = exoPlayer.getVideoDecoderCounters();
    if (counters != null) {
      counters.ensureUpdated();
      values.put("droppedFrames", counters.droppedBufferCount);
      values.put(
          "totalFrames",
