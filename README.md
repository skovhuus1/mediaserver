...
- 🛠️ GitHub Actions eksporterer nu isolerede CI-værdier for PostgreSQL, Redis og applikationssecrets til alle migrations-, test- og build-steps; den initiale SQL-migration er normaliseret til BOM-fri UTF-8, og `shared/types` følger TypeScripts composite/incremental-kontrakt. Fuld grøn pipeline afrapporteres først efter verificeret run.
