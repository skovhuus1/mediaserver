        ...
        <article className={styles.hero} style={{ backgroundImage: data.hero.backdropPath ? `linear-gradient(90deg,rgba(5,8,12,.97),rgba(5,8,12,.18)),url("${data.hero.backdropPath}")` : undefined }}>
          <div>
            ...
            <div className={styles.actions}>
              <Link href={`/watch?play=${data.hero.id}`}><Play size={17} fill="currentColor" />Afspil</Link>
              <Link href={`/watch?info=${data.hero.id}`}><Info size={17} />Info</Link>
              {controls(data.hero)}
            </div>
          </div>
        </article>
