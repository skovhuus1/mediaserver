...
type PlaybackQueueState = { paused: boolean; effectivePaused: boolean; pauseReason: 'manual' | 'schedule' | null; queued: number; running: number; pausedJobs: number; orphaned: number; scheduleEnabled: boolean; scheduleOpen: boolean };
...
export function PlaybackAnalysis() {
  ...
  async function recoverOrphaned() { setBatchBusy(true); setMessage(''); try { const result = await api<{ recovered: number; remaining: number }>('/playback-analysis/queue/recover', { method: 'POST' }); setMessage(`${result.recovered} fastlåste analyser er sat sikkert i kø igen${result.remaining ? ` · ${result.remaining} afventer næste recovery` : ''}.`); await loadRows(); } catch (error) { setMessage(errorMessage(error)); } finally { setBatchBusy(false); } }
  ...
  return <section className={styles.page}>
    ...
    {(queueState?.orphaned ?? 0) > 0 && <section className={styles.recoveryNotice}><AlertTriangle size={20} /><div><strong>{queueState?.orphaned.toLocaleString('da-DK')} fastlåste analyser</strong><p>Asset-status findes, men workerjobbet mangler. Genkør kun disse analyser uden at starte resten af det manglende katalog.</p></div><button type="button" disabled={!canWrite || batchBusy} onClick={() => void recoverOrphaned()}>{batchBusy ? 'Genopretter...' : 'Genkør fastlåste'}</button></section>}
