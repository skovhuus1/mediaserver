...
type TaskFeed = { summary: { total: number; queued: number; running: number; paused: number; completed: number; failed: number }; items: Task[]; sampledAt: string };
...
export function OperationsCenter() {
  ...
  const [playbackQueue, setPlaybackQueue] = useState<PlaybackQueueState | null>(null);
  ...
  const load = useCallback(async () => {
    const [nextFeed, nextLibraries, nextWatcher, nextPlaybackQueue] = await Promise.all([api<TaskFeed>('/system/jobs'), api<Library[]>('/libraries'), api<WatcherStatus>('/system/library-watcher/status'), api<PlaybackQueueState>('/playback-analysis/queue/status')]);
    setFeed(nextFeed); setLibraries(nextLibraries); setWatcher(nextWatcher); setPlaybackQueue(nextPlaybackQueue); setLibraryId((current) => current || nextLibraries[0]?.id || '');
  }, []);
  useEffect(() => {
  ...
  const visible = useMemo(() => (feed?.items ?? []).filter((task) => filter === 'active' ? ['queued', 'running', 'paused'].includes(task.status) : filter === 'all' ? true : task.status === filter), [feed, filter]);
  ...
  return <section className={styles.page}>
    ...
    <div className={styles.summary}><Metric icon={<LoaderCircle />} label="Kører" value={feed?.summary.running ?? 0} tone="active" /><Metric icon={<Clock3 />} label="I kø" value={feed?.summary.queued ?? 0} tone="queued" /><Metric icon={<Clock3 />} label="Pauset" value={feed?.summary.paused ?? 0} tone="queued" /><Metric icon={<CheckCircle2 />} label="Fuldført" value={feed?.summary.completed ?? 0} tone="done" /><Metric icon={<AlertTriangle />} label="Fejlet" value={feed?.summary.failed ?? 0} tone="failed" /></div>
    ...
    <section className={styles.launchpad}><header><div><span>START NY OPGAVE</span><h2>Vedligehold biblioteket</h2></div><small>Opgaver kører i worker-køen og fortsætter, hvis du forlader siden.</small></header><div className={styles.actions}>
      ...
      <article><Film /><div><strong>Playback-analyse</strong><p>{playbackQueue?.pauseReason === 'manual' ? `${playbackQueue.pausedJobs} analyser er manuelt pauset. Aktive filer afsluttes sikkert.` : playbackQueue?.pauseReason === 'schedule' ? `${playbackQueue.queued} analyser venter på næste kørselsvindue.` : 'Byg seek-preview og find intro, recap og rulletekst.'}</p><Link href="/?admin=playback-analysis">Redigér tidsplan</Link></div><div className={styles.doubleSelect}><select value={analysisScope} onChange={(event) => setAnalysisScope(event.target.value as typeof analysisScope)}><option value="all">Film og serier</option><option value="movie">Kun film</option><option value="series">Kun serier</option></select><select value={analysisMode} onChange={(event) => setAnalysisMode(event.target.value as typeof analysisMode)}><option value="missing">Kun manglende</option><option value="all">Genopbyg alle</option></select></div><button disabled={!canWrite || busy !== null || playbackQueue?.paused} onClick={() => void run('analysis', () => api('/media/playback-assets/jobs', { method: 'POST', body: JSON.stringify({ mediaType: analysisScope, mode: analysisMode }) }), playbackQueue?.pauseReason === 'schedule' ? 'Playback-analyserne er sat i kø til næste kørselsvindue.' : 'Playback-analyserne er sat i kø.')}>{busy === 'analysis' ? 'Starter...' : 'Start analyse'}</button><button disabled={!canWrite || busy !== null} onClick={() => void run('analysis-pause', () => api(`/playback-analysis/queue/${playbackQueue?.paused ? 'resume' : 'pause'}`, { method: 'POST' }), playbackQueue?.paused ? 'Den manuelle pause er fjernet.' : 'Playback-analysen er manuelt pauset efter aktive filer.')}>{busy === 'analysis-pause' ? 'Arbejder...' : playbackQueue?.paused ? 'Fjern manuel pause' : 'Pause generering'}</button></article>
...
function taskLabel(type: string) { return ({ 'library.scan': 'Biblioteksscan', 'media.metadata': 'Metadata', 'media.playback-assets': 'Playback-analyse', 'playback.transcode': 'Transcoding', 'offline.prepare': 'Offline-klargøring', 'push.deliver': 'Push-notifikation', 'live-tv.import': 'Live TV M3U-import', 'live-tv.epg': 'Live TV XMLTV-guide', 'live-tv.channel-visibility': 'Live TV-kanalændring', 'live-tv.stream': 'Live TV-stream', 'live-tv.record': 'Live TV-optagelse' } as Record<string, string>)[type] ?? type; }
function statusLabel(status: string) { return ({ queued: 'I kø', running: 'Kører', paused: 'Pauset', completed: 'Fuldført', failed: 'Fejlet' } as Record<string, string>)[status] ?? status; }
