...
export class OperationalTelemetryService implements OnModuleInit, OnModuleDestroy {
  ...
  private async sampleAccount(accountId: string) {
    ...
    await this.prisma.$executeRaw(Prisma.sql`
      INSERT INTO "system_metric_samples" (
        ...
        ${randomUUID()}::uuid, ${accountId}::uuid, ${now}, ${point.cpuPercent}, ${point.memoryPercent}, ${BigInt(memoryUsedBytes)}, ${BigInt(memoryTotalBytes)},
  ...
  private async syncAlerts(accountId: string, point: MetricPoint) {
    ...
    for (const candidate of candidates) {
      await this.prisma.$executeRaw(Prisma.sql`
        ...
        VALUES (${randomUUID()}::uuid, ${accountId}::uuid, ${candidate.key}, ${candidate.severity}, ${candidate.title}, ${candidate.message}, ${JSON.stringify(candidate.details)}::jsonb)
    ...
    if (activeKeys.length) {
      await this.prisma.$executeRaw(Prisma.sql`UPDATE "system_alert_events" SET "status" = 'resolved', "resolved_at" = NOW(), "last_seen_at" = NOW() WHERE "account_id" = ${accountId}::uuid AND "status" IN ('open', 'acknowledged') AND NOT ("alert_key" = ANY(${activeKeys}::text[]))`);
    } else {
      await this.prisma.$executeRaw(Prisma.sql`UPDATE "system_alert_events" SET "status" = 'resolved', "resolved_at" = NOW(), "last_seen_at" = NOW() WHERE "account_id" = ${accountId}::uuid AND "status" IN ('open', 'acknowledged')`);
    }
  }
  ...
  async history(accountId: string, range?: string) {
    ...
    const rows = await this.prisma.$queryRaw<MetricRow[]>(Prisma.sql`
      ...
      FROM "system_metric_samples" WHERE "account_id" = ${accountId}::uuid AND "sampled_at" >= ${from} ORDER BY "sampled_at" ASC
    `);
  ...
  async alerts(accountId: string) {
    return this.prisma.$queryRaw(Prisma.sql`
      ...
      FROM "system_alert_events" WHERE "account_id" = ${accountId}::uuid ORDER BY ("status" = 'resolved') ASC, "last_seen_at" DESC LIMIT 100
    `);
  }
  ...
  async acknowledge(accountId: string, userId: string, id: string) {
    const rows = await this.prisma.$queryRaw<Array<{ id: string; status: string }>>(Prisma.sql`
      UPDATE "system_alert_events" SET "status" = 'acknowledged', "acknowledged_at" = NOW(), "acknowledged_by" = ${userId}::uuid
      WHERE "id" = ${id}::uuid AND "account_id" = ${accountId}::uuid AND "status" = 'open' RETURNING "id", "status"
    `);
