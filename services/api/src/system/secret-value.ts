...
export function decryptSecret(value: unknown, configuredKey = process.env.ENCRYPTION_KEY): string {
  ...
  const decipher = createDecipheriv('aes-256-gcm', encryptionKey(configuredKey), Buffer.from(value.iv, 'base64'));
...
function encryptionKey(value: string | undefined): Buffer {
  if (!value?.startsWith('base64:')) throw new Error('ENCRYPTION_KEY is not configured');
