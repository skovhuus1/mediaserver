...
describe('experience utilities', () => {
  ...
  it('builds ordered seasons, resume state and the next unwatched episode', () => {
    ...
    expect(result.seasons[0]?.watchedCount).toBe(0);
    expect(result.resumeEpisode?.id).toBe('e2');
    expect(result.nextEpisode?.id).toBe('e3');
  });

  it('only treats episodes as watched when history is explicitly completed', () => {
    const episodes = [
      { id: 'e1', title: 'En', overview: null, seasonNumber: 1, episodeNumber: 1, releaseYear: 2024, stillPath: null, posterPath: null, durationMs: 1_000, markers: [], playback: {} },
      { id: 'e2', title: 'To', overview: null, seasonNumber: 1, episodeNumber: 2, releaseYear: 2024, stillPath: null, posterPath: null, durationMs: 1_000, markers: [], playback: {} },
    ];
    const almostFinished = buildSeriesSeasons(episodes, [
      { mediaId: 'e1', positionMs: 999, completed: false, updatedAt: new Date('2026-01-01') },
    ], 'e1');
    expect(almostFinished.seasons[0]?.watchedCount).toBe(0);
    expect(almostFinished.resumeEpisode?.id).toBe('e1');

    const completed = buildSeriesSeasons(episodes, [
      { mediaId: 'e1', positionMs: 1_000, completed: true, updatedAt: new Date('2026-01-01') },
    ], 'e1');
    expect(completed.seasons[0]?.watchedCount).toBe(1);
    expect(completed.resumeEpisode?.id).toBe('e2');
  });
